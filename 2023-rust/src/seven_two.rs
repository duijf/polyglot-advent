...
fn classify(hand: &[Card]) -> Classification {
    ...
    let num_jokers: u64 = *cards_by_amount.get(&Card::Joker).unwrap_or(&0);

    let mut amounts: Vec<_> = cards_by_amount.values().collect();
    amounts.sort();

    match (&amounts[..], num_jokers) {
        (&[5], _) => Classification::FiveOfAKind,

        (&[1, 4], 1 | 4) => Classification::FiveOfAKind,
        (&[1, 4], 0) => Classification::FourOfAKind,

        (&[2, 3], 2 | 3) => Classification::FiveOfAKind,
        (&[2, 3], 0) => Classification::FullHouse,

        (&[1, 1, 3], 1 | 3) => Classification::FourOfAKind,
        (&[1, 1, 3], 0) => Classification::ThreeOfAKind,

        (&[1, 2, 2], 1) => Classification::FullHouse,
        (&[1, 2, 2], 2) => Classification::FourOfAKind,
        (&[1, 2, 2], 0) => Classification::TwoPair,

        (&[1, 1, 1, 2], 1 | 2) => Classification::ThreeOfAKind,
        (&[1, 1, 1, 2], 0) => Classification::OnePair,

        (&[1, 1, 1, 1, 1], 1) => Classification::OnePair,
        (&[1, 1, 1, 1, 1], 0) => Classification::HighCard,
        _ => unreachable!(),
    }
}
...
mod tests {
    ...
    fn classification() {
        ...
        assert_eq!(classify(&[Ace, Ace, King, Two, Queen]), OnePair);
        assert_eq!(classify(&[Ace, Two, King, Three, Queen]), HighCard);
        assert_eq!(classify(&[Ace, Two, Three, Ace, Four]), OnePair);
        assert_eq!(classify(&[Two, Three, Four, Three, Two]), TwoPair);

        assert_eq!(classify(&[Ace, Ace, King, Joker, Queen]), ThreeOfAKind);
        assert_eq!(classify(&[Ten, Five, Five, Joker, Five]), FourOfAKind);
        assert_eq!(classify(&[King, Ten, Joker, Joker, Ten]), FourOfAKind);
        assert_eq!(classify(&[Queen, Queen, Queen, Joker, Ace]), FourOfAKind);
    }
    ...
    fn comparison() {
        ...
        assert_ordering(
            vec![Joker, King, King, King, Two],
            vec![Queen, Queen, Queen, Queen, Two],
            Less,
        );
    }
